...
import { Container, Row, Column, Icon, ButtonGroup , Button, ScrollView, ChosenPicker,Checkbox } from 'app/components'
...
import * as syxmbActions from 'app/redux/Report/Syxmb/syxmb.action.js'
...
class Ambsyb extends React.Component {
	...
	componentWillUnmount() {
		this.props.dispatch(syxmbActions.changeCharDidmount(false))
		this.props.dispatch(syxmbActions.switchCharStatus('损益'))
	}
	...
	render() {
		...
		const view = syxmbState.get('view')
		...
		const initCardList= syxmbState.get('initCardList')
		const cardUuid = syxmbState.get('cardUuid')
		const beCategory = syxmbState.get('beCategory')
		const needCategory = syxmbState.get('needCategory')
		const showCheck = syxmbState.get('showCheck')
		const cardName = syxmbState.get('cardName')
		// 当前的辅助类别
		const assCategory = view.get('assCategory')
		// 当前的辅助项目
		const assList = assCategory ? syxmbState.get('assList').find(v => v.get('asscategory') === assCategory).get('asslist') : fromJS([])
		...
		// 类别可选列表
		const AsscategoryList = syxmbState.get('assList').map(v => {
			return {
		...
		const ddExcelCallback = () => dispatch => dispatch(allActions.allExportDo('excelAMBSYBIncome', {begin: begin, end: end, cardUuid, beCategory, needCategory}))
			...
			<Container className="jz ass-config">
				<TopMonthPicker
					...
					callback={(value) => {//左右icon的作用
						dispatch(syxmbActions.getSyxmbCardList(value, endissuedate, "false"))
						dispatch(syxmbActions.changeNeedCategory(false))
						// this.setState({isLineChar: !isLineChar})
						this.props.dispatch(syxmbActions.switchCharStatus('损益'))
					}}
					onOk={(result) => {//单期选择完起始时间后
						dispatch(syxmbActions.getSyxmbCardList(result.value, endissuedate,'false'))
						dispatch(syxmbActions.changeNeedCategory(false))
						// this.setState({isLineChar: !isLineChar})
						this.props.dispatch(syxmbActions.switchCharStatus('损益'))
					}}
					...
					onBeginOk={(result) => {//跨期选择完开始时间后
						...
						dispatch(syxmbActions.getSyxmbCardList(result.value, '', 'false'))
						dispatch(syxmbActions.changeNeedCategory(false))
						this.props.dispatch(syxmbActions.switchCharStatus('损益'))
					}}
					onEndOk={(result) => {//跨期选择完结束时间后
						dispatch(syxmbActions.getSyxmbCardList(issuedate, result.value, 'false'))
						dispatch(syxmbActions.changeNeedCategory(false))
						this.props.dispatch(syxmbActions.switchCharStatus('损益'))
					}}
					changeEndToBegin={()=>{//跨期变为单期之后 使endissuedate为空 重新获取数据
						dispatch(syxmbActions.getSyxmbCardList(issuedate, '', 'false'))
						dispatch(syxmbActions.changeNeedCategory(false))
					}}
				/>
				<Row className='select-row'>
					<ChosenPicker
						className='card-list'
						district={cardList.toJS()}
						parentDisabled={false}
						value={cardUuid}
						onChange={(result) => {
							let item
							const loop = (initCardList)=>initCardList.map((e)=>{
								if(e.uuid===result.key){
									item=e
								}
							...
							loop(initCardList)
							dispatch(syxmbActions.setCardName(result.label))
							dispatch(syxmbActions.getAMBIncomeStatementFetch(issuedate, endissuedate, item.uuid,item.beCategory, needCategory))
							this.props.dispatch(syxmbActions.switchCharStatus('损益'))
							if(result.label==='全部'){
								if (initCardList.some(v => v.uuid && v.beCategory)) {
									dispatch(syxmbActions.handleCheckBoxShow(true))
								} else {
									dispatch(syxmbActions.handleCheckBoxShow(false))
								}
							}else{
					...
					>
						<Row className="lrbselect">
							<span className={["lrbselect-assmane", disableTime ? 'lrb-item-disable' : ''].join(' ')}>{cardName}</span>
							<Icon className="lrbselect-icon" type="triangle" size="11"></Icon>
						</Row>
					</ChosenPicker>
					{showCheck &&
						<div className='check-needCategory' onClick={(e)=>{
							dispatch(syxmbActions.changeNeedCategory(!needCategory))
							dispatch(syxmbActions.getAMBIncomeStatementFetch(issuedate, endissuedate, cardUuid,beCategory, !needCategory))
						}}>
							<Checkbox
				...
				<ScrollView className="content" flex='1'>
					...
					<div className={"ambysb-charts-wrap"}>
						...
						<div
							...
							onClick={() => dispatch(syxmbActions.switchCharStatus(nextStatus))}
							>
					...
					<div className="ambysb-charts-wrap">
						...
						{didMount ?
							(status === '损益' &&  beCategory===false?
								// <div className="ambysb-ambbarchar-tip">损益由收入与支出作差得出，无明细构成</div> :
