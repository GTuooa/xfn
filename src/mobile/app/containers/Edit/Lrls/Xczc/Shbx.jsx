...
class Shbx extends React.Component {
	...
	render () {
		...
		const { bePersonSocialSecurity, showList } = this.state
					...
					amountCom = <div className='lrls-card'>
						...
						<Row className='yysr-amount'>
							...
							<TextListInput
								...
								onChange={(value) => {
									if (/^\d*\.?\d{0,2}$/g.test(value)) {
										dispatch(xczcAccountActions.xczcPersonAmount('companySocialSecurityAmount',value))
								...
								<Switch
									...
									onClick={() => {
										if (bePersonSocialSecurity) {//变为不代缴
											...
											dispatch(xczcAccountActions.xczcPersonAmount('personSocialSecurityAmount', 0))
											dispatch(xczcAccountActions.autoCalculateAmount(propertyPay))
										} else {
		...
		if (beAccrued && runningState === 'STATE_XC_JN' && showHs) {
			HxCom = xczcCom('待发放社会保险', jtAmount, paymentList, '计提社会保险')
		}
