...
class Zskx extends React.Component {
	...
	render () {
		const { dispatch, zskxAccountState } = this.props
		const paymentList = zskxAccountState.getIn(['data', 'paymentList'])
		...
		return(
			<Container className="lrls">
						...
						paymentList.map((v, i) => {
							return (
								<Row
									...
									onClick={() => {
										if (v.get('beSelect')) {
											dispatch(zskxAccountActions.paymentListBeSelect(i, false))
										} else {
											dispatch(zskxAccountActions.paymentListBeSelect(i, true))
										}
									}}
								>
									...
									<div className='xczc-ls-line xczc-ls-bottom'>
										<span className='xczc-ls-color'>暂收款项</span>
									</div>
								</Row>
							)
						})
				...
				<ButtonGroup style={{backgroundColor: '#F8F8F8'}}>
					<Button onClick={() => {
						dispatch(homeAccountActions.changeHomeAccountData('categoryType','LB_ZSKX'))
						dispatch(zskxAccountActions.changeZskxData('amount', totalAmount))
						dispatch(zskxAccountActions.changeZskxData('jtAmount', totalAmount))
					}}>
						<Icon type="choose"/>
						<span>确定</span>
					</Button>
