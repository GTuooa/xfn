...
@connect(state => state)
export default
class ZbLs extends React.Component {

	render () {
		const { dispatch, zbAccountState } = this.props
		const paymentList = zbAccountState.getIn(['data', 'paymentList'])
		...
		return(
			<Container className="lrls">
						...
						paymentList.map((v, i) => {
							return (
								<Row
									...
									onClick={() => {
										if (v.get('beSelect')) {
											dispatch(zbAccountActions.paymentListBeSelect(i, false))
										} else {
											dispatch(zbAccountActions.paymentListBeSelect(i, true))
										}
									}}
								>
									...
									<div className='xczc-ls-line xczc-ls-bottom'>
										<span className='xczc-ls-color'>利润分配</span>
									</div>
								</Row>
							)
						})
				...
				<ButtonGroup>
					<Button onClick={() => {
						dispatch(homeAccountActions.changeHomeAccountData('categoryType','LB_ZB'))
						dispatch(zbAccountActions.changeZbData('amount', totalAmount))
						dispatch(zbAccountActions.changeZbData('jtAmount', totalAmount))
					}}>
						<Icon type="choose"/>
						<span>确定</span>
					</Button>
