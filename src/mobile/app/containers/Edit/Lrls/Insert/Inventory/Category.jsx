...
class Relation extends React.Component {
	...
	render () {
		...
		const treeIdx = homeAccountState.getIn(['inventory', 'treeIdx'])
		const cardTypeTree = homeAccountState.getIn(['inventoryTreeList', treeIdx, 'value'])
			...
			if (item.get('childList') && item.get('childList').size) {
				...
				return (
					<div key={item.get('uuid')}
						...
						onClick={() => {
							dispatch(homeAccountActions.changeLrlsData(['inventory', 'psiData', 'categoryTypeList', treeIdx, 'subordinateUuid'], item.get('uuid')))
							dispatch(homeAccountActions.changeLrlsData(['inventory', 'psiData', 'categoryTypeList', treeIdx, 'subordinateName'], item.get('name')))
							history.goBack()
						}}
					>
						{item.get('name')}
		...
		return(
			<Container className="inventory-config iuManage-lrls">
