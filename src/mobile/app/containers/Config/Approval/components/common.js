...
//数字校验
export const numberTest = (value,dispatchFunc,NegativeAllowed) => {
    if (value === undefined)
        return
    ...
    if (value.indexOf('-') === 0 && !NegativeAllowed) {
        ...
        thirdParty.toast.info('金额只能输入小于14位数两位小数的数字')
    } else {
        dispatchFunc(value)
    }
}
...
function getSelectJrCategoryList(allCategoryList, hideCategoryList, parentDisabled, currentInfo) { // 筛选出可选的流水类别
    ...
    if (allCategoryList && allCategoryList.getIn([0, 'childList']).size > 0) {
        ...
        if (indexYYSR > -1) {
            // selectCategoryList = selectCategoryList.updateIn([indexYYSR, 'childList'], v => v.filter(w => w.get('propertyCarryover') == 'SX_FW'))
            if (currentInfo.payOrReceive) { // 有值说明是查询审批调整框来的
                // if (propertyCarryover === 'SX_HW' || (propertyCarryover === 'SX_HW_FW' && isStock)) {
                if (currentInfo.isStock) {
        ...
        //             childList: loop(childList, leve+1)
        //         }
        //     } else {
        //         return {
        //             title: item.name,
        //             value: item.uuid + Limit.TREE_JOIN_STR + item.categoryType + Limit.TREE_JOIN_STR + item.name,
        //             disabled: (item.categoryType === 'LB_YYWSR' || item.categoryType === 'LB_YYWZC') ? false : leve === 1,  // 最外面的类别如果没有子类别不可选, 除了营业外收支
        //             key: item.uuid,
        //         }
        //     }
        // })

        // const selectTreeList = loop(selectCategoryList.toJS(), 1)
        // return selectTreeList
        return selectCategoryList.toJS()
    } else {
        return []
    }
}

function getSelectJrCategoryChildList(parentUuid, allCategoryList, parentName) {

    if (allCategoryList && allCategoryList.getIn([0, 'childList']).size > 0 && parentUuid) {

        const canSelectCategoryType = ['LB_YYSR', 'LB_YYZC', 'LB_FYZC', 'LB_XCZC', 'LB_YYWSR', 'LB_YYWZC'] // 所有可选类别
        let selectCategoryList = allCategoryList.getIn([0, 'childList']).filter(v => canSelectCategoryType.indexOf(v.get('categoryType')) > -1)
...
function formatWarehouseCardList (warehouseCardList) {
    ...
    if (warehouseCardList.size && warehouseCardList.getIn([0, 'childList']).size > 0) {

        const selectList = warehouseCardList.getIn([0, 'childList'])
...
export { formDiyList, getSelectJrCategoryList, componentName, getSelectJrCategoryChildList, formatWarehouseCardList, formatCommonCardList }
