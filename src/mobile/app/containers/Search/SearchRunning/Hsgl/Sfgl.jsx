...
class Sfgl extends React.Component {
	componentDidMount() {
		thirdParty.setTitle({ title: '收付管理' })
	...
	render () {
		...
		const currentCard = data.getIn(['currentCardList', 0])
			...
			<Container className="edit-running">
				...
				<ScrollView flex="1">
					...
					<div className='lrls-card'>
						{
							currentCard ? <div className='lrls-bottom'>
								往来单位： {`${currentCard.get('code')} ${currentCard.get('name')}`}
							</div> : ''
						}
					...
					{
						amountTitle.includes('收款') && accountPoundage.get('uuid') ? <Poundage
							dispatch={dispatch}
							data={data}
							accounts={accounts}
				...
				<ButtonGroup style={{backgroundColor: '#F8F8F8'}}>
					...
					<Button onClick={() => {
						dispatch(searchRunningActions.saveSfgl(history))
					}}>
						<Icon type="new"/>
						<span>保存</span>
					</Button>
