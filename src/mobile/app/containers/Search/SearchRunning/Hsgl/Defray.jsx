...
import { XfInput, Row, Icon, Button, ButtonGroup, Container, ScrollView, TextareaItem, Single } from 'app/components'
...
@connect(state => state)
export default
class Defray extends React.Component {
	componentDidMount() {
		...
		return(
			<Container className="edit-running">
					...
					<div className='lrls-card'>
						<Row className='yysr-amount'>
							...
							<XfInput.BorderInputItem
								...
								value={amountValue}
								onChange={(value) => {
									if (oriState=='STATE_XC_JN') {//工资薪金
										dispatch(searchRunningActions.changeCxlsData(['data', 'payment', 'actualAmount'], value))
										const propertyPay = data.get('propertyPay')
										if (propertyPay=='SX_SHBX') {
											dispatch(searchRunningActions.changeCxlsData(['data', 'payment', 'companySocialSecurityAmount'], value))
										}
										if (propertyPay=='SX_ZFGJJ') {
											dispatch(searchRunningActions.changeCxlsData(['data', 'payment', 'companyAccumulationAmount'], value))
										}
									} else {//税费支出
				...
				<ButtonGroup style={{backgroundColor: '#F8F8F8'}}>
					...
					<Button onClick={() => {
						dispatch(searchRunningActions.saveDefray(history))
					}}>
						<Icon type="new"/>
						<span>保存</span>
					</Button>
