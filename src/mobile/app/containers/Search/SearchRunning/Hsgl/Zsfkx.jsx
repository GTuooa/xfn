...
class Xczc extends React.Component {
	componentDidMount() {
		const oriState = this.props.searchRunningState.getIn(['data', 'oriState'])
		thirdParty.setTitle({ title: oriState == 'STATE_ZS_TH' ? '付款核销' : '收款核销' })
		...
		return(
			<Container className="edit-running">
				...
				<ScrollView flex="1">
					...
					<div className='lrls-card'>
						<Row className='yysr-amount'>
							<label>{oriState == 'STATE_ZS_TH' ? '付款金额' : '收款金额'}</label>
					...
					{
						oriState != 'STATE_ZS_TH' && accountPoundage.get('uuid') ? <Poundage
							dispatch={dispatch}
							data={data}
							accounts={accounts}
				...
				<ButtonGroup style={{backgroundColor: '#F8F8F8'}}>
					...
					<Button onClick={() => {
						dispatch(searchRunningActions.saveZsfkx(history))
					}}>
						<Icon type="new"/>
						<span>保存</span>
					</Button>
