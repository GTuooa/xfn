...
class UploadEnclosure extends React.Component {
	...
	componentDidMount() {
        ...
        var uploader = new plupload.Uploader({
            browse_button : that.props.id === undefined ? 'browse' : that.props.id, //触发文件选择对话框的按钮，为那个元素id
		...
		uploader.bind('FilesAdded', function(uploader, files) {
			...
			if (files.length > 0) {
				...
				if (checkedAllFileIsOk) { // 校验没有问题后
					...
					if (that.state.isClipBoardUpload) { // 剪贴版上传不走压缩那一套
		...
		uploader.bind('FileUploaded', function(uploader, files, info) {
			...
			if (info.status == 200) {
				that.props.uploadEnclosureList([files.name, files, JSON.parse(info.response), index===fileSize])
		...
		uploader.bind('Error', function(uploader, err) {
			if (err.code == -600) {
				...
				if (err.status === 0) {
					...
					thirdParty.Alert("上传出错:" + err.response, '确定')
				}
				// thirdParty.Alert("上传出错", '确定')
				xfnFetchErrorToDeveloper({
                    title: '上传出错',
                    message: `${err.status}-${err.code}-${err.message}-${err.requestId}-${JSON.stringify(err.file)}`,
                    remark: '附件'
		...
		this.clipUploadFile = (clipFile) => {
			uploader.addFile(clipFile);
		}
	}
	...
	render() {
		...
		const { uploadingModal, errMessage, secondUpload, isClipBoardUpload } = this.state
			...
			<Fragment>
				...
				<ClipBoard
					...
					clipUploadFile={(file) => {
						this.setState({
							secondUpload: false,
							isClipBoardUpload: true
						})
						this.clipUploadFile(file)
					}}
					formPage={formPage}
				/>
			</Fragment>
