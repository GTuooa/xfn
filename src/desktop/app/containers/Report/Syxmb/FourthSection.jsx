...
class FourthSection extends React.Component {
	...
	render() {
		...
		const { cardName, ambDetailTable, tableShowChild, dispatch, scrollTop, offsetTop, showAssTable, onSeclect, isSpread,unit } = this.props
			...
			<div className="ambsyb-fourthsection">
				<div className="ambsyb-fourthsection-title">
					{showAssTable ?
						...
						<h4 className="ambsyb-fourthsection-title-title">{cardName}损益表</h4>
					}
				</div>
					...
					</ul>
					{
						acTable.map((v, i) => {
							...
							return <ul
								...
								>
								<li onClick={() => v.get('acId').length >= 4 && dispatch(syxmbActions.changeTableShowChild(v.get('acId')))}>
									<span
