...
class AmbPieChar extends React.Component {
	...
	render() {
		...
		const income = gainAndLoss.get('income') > 0 ? gainAndLoss.get('income')/unit : 0
		const pay = gainAndLoss.get('pay') > 0 ? gainAndLoss.get('pay')/unit : 0
		const ginAndLoss = gainAndLoss.get('ginAndLoss')/unit
