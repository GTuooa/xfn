    ...
    render() {
        ...
        return (
            <Modal
              ...
              onOk={() => {
                  cancel()
                  // dispatch(innerCalculateActions.changeEditCalculateCommonString('CommonCharge','projectCardList',projectCard.concat(selectItem)))
                  dispatch(editCalculateActions.changeCommonChargeProject(projectCard.concat(selectItem)))
                      ...
                      <div className="table-title-wrap">
                          <ul className="table-title table-title-charge">
                              <li
                                  onClick={(e) => {
                                      e.stopPropagation()
                                      showSelectAll && dispatch(editCalculateActions.changeItemCheckboxCheckAll(checkedAll,newThinglist,thingsList))
                                  }}
                              >
                                  {
                                      showSelectAll ?
                      ...
                      <TableBody style={{height:thingsList && thingsList.toJS() > 5?'374px':'286px'}}>
                          {
                              thingsList && thingsList.toJS().map(v => {
                                  ...
                                  const disabled = projectCard.some(w => w.get('cardUuid') === v.uuid) && ((oriState === 'STATE_FZSCCB' || oriState === 'STATE_ZZFY') && v.code !== 'COMNCRD')
