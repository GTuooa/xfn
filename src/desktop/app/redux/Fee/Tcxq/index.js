...
import { judgeIsInnerOneYear } from 'app/utils'
	...
	return ({
		[ActionTypes.GET_ADMIN_CORPINFO_EQUITY]			   : () => {
			...
			state.getIn(['data', 'corpInfo', 'orderInfoList']).map((v,i) => {   //把所有发票的数据存入orderItemStatus中包括indexId，isInvoice,orderNo
				...
				const orderTime = v.get('orderTime')
				const orderDay = orderTime.split(' ')
				const isInnerOneYear = judgeIsInnerOneYear(orderDay[0])

                if (v.get('isInvoice') == "NO" && v.get('tradeStatus') !== 'WAIT_BUYER_PAY' && v.get('isRefund') == "NO" && isInnerOneYear){
